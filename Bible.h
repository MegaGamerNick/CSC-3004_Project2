...
{	
   ...
   public:
     ...
     void debugIndex() const;                  // prints info about the index
     int getIndexSize() const;                 // number of entries in the index
     pair<Ref,int> getLastIndexEntry() const;  // last entry in the index
     int getOffset(const Ref& r) const;       // byte offset for a reference
};
